...
async function reading(u,root){
 const{data,error}=await supabase.from('reading_progress').select('content_id,progress_percent,last_read_at,content_items(id,title,type,cover_image_url,read_time_minutes)').eq('user_id',u.id).order('last_read_at',{ascending:false});
...
async function recent(u,root){
 ...
 root.innerHTML=itemRows(data,'Open');wireItems(root);
}
async function collections(u,root){
 ...
 root.innerHTML=`<form class="collection-bar"><input name="name" placeholder="New collection name" maxlength="80" required><button>Create</button></form><div class="collection-grid">${(data||[]).map(c=>`<button class="collection-card" data-collection-id="${esc(c.id)}"><strong>${esc(c.name)}</strong><small>${esc(c.description||'Personal collection')}</small></button>`).join('')||'<div class="library-empty">Create your first collection.</div>'}</div>`;
 root.querySelector('form').onsubmit=async e=>{e.preventDefault();const name=new FormData(e.currentTarget).get('name')?.trim();if(!name)return;await supabase.from('collections').insert({user_id:u.id,name});loadLibrary()};root.querySelectorAll('[data-collection-id]').forEach(b=>b.onclick=()=>collectionDetail(u,b.dataset.collectionId,root));
}
